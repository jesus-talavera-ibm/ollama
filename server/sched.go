...
func (s *Scheduler) load(req *LlmRequest, systemInfo ml.SystemInfo, gpus []ml.DeviceInfo, requireFull bool) bool {
	...
	if llama == nil {
		var err error
		isMLX := req.model.IsMLX() || slices.Contains(req.model.Config.Capabilities, "image")
		if !isMLX {
			...
			if slices.Contains(req.model.Config.Capabilities, "image") {
				llama, err = imagegen.NewServer(modelName, imagegen.ModeImageGen)
			} else if req.useImagegen {
				llama, err = imagegen.NewServer(modelName, imagegen.ModeLLM)
			} else {
...
iGPUScan:
	...
	runner := &runnerRef{
		...
		isImagegen:      req.useImagegen || slices.Contains(req.model.Config.Capabilities, "image"),
		totalSize:       totalSize,
		vramSize:        vramSize,
		loading:         true,
		pid:             llama.Pid(),
