...
// NewServer prepares a new MLX runner server for image generation or LLM models.
...
func (s *Server) Load(ctx context.Context, _ ml.SystemInfo, gpus []ml.DeviceInfo, requireFull bool) ([]ml.DeviceID, error) {
	...
	slog.Info("starting mlx runner subprocess", "model", s.modelName, "port", s.port, "mode", s.mode)
